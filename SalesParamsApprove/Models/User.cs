...
namespace SalesParamsApprove.Models
{
    public class User
    {
        ...
        public User(int idUser, string n_kontr_full)
        {
            ...
            this.UserDomainName = GetUserDomainName();
        }

        public static string GetUserDomainName()
        {
            string sql = @"select SUSER_NAME()";
            return DBExecute.SelectScalar(sql).ToString();
        }

        public static int GetPostByUserId(int idUser)
        {
            string sql = @"select id_post from spr_kontr where id_kontr = @idKontr";
            SqlParameter paridKontr = new SqlParameter("idKontr", idUser);
            return int.Parse(DBExecute.SelectScalar(sql, paridKontr).ToString());
        }
        ...
        public static bool InRole(int idUser, string NRole)
        {
            ...
            var res = DBExecute.SelectScalar(sql, paridKontr, parRoleName);
